...
query = ('''CREATE TABLE IF NOT EXISTS Ten_Students(ID INT NOT NULL PRIMARY KEY, First_name TEXT, Last_name TEXT, Midterm REAL, 
        Final REAL, Project1 REAL, Project2 REAL, Grade REAL, Letter_Grade TEXT);''')
...
query = ('''INSERT INTO Ten_Students VALUES
        ...
        ('111120', 'Anthony', 'White', '92', '100.0', '95', '92', '94.5', 'A');''')
cursor.execute(query)
db.commit()
query = ('''SELECT * FROM Ten_Students ORDER BY First_name;''')
print(cursor.execute(query).fetchall())
query = ('''SELECT Letter_Grade, COUNT(*) FROM Ten_Students GROUP BY Letter_Grade;''')
print(cursor.execute(query).fetchall())
query = ('''DELETE FROM Ten_Students WHERE Letter_Grade = 'D';''')
cursor.execute(query)
db.commit()
db.close()
